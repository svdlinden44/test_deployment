...
import { getMyFavoriteRecipes } from '@/lib/api/endpoints'
...
export function Favorites() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      setError(null)
      try {
        const res = await getMyFavoriteRecipes(1, PER_PAGE)
        if (cancelled) return
        setRecipes(res.results.map((r) => ({ ...r, is_favorited: true })))
  ...
  const loadMore = useCallback(async () => {
    ...
    setLoadingMore(true)
    try {
      const res = await getMyFavoriteRecipes(page, PER_PAGE)
      setRecipes((prev) => [
        ...prev,
        ...res.results.map((r) => ({ ...r, is_favorited: true })),
      ])
      const more = Boolean(res.next)
      setHasMore(more)
      hasMoreRef.current = more
  ...
  return (
    <div className={s.page}>
      <div className={s.hero}>
        <span className={s.label}>Saved</span>
        <h1 className={s.title}>Favorites</h1>
        <p className={s.sub}>Cocktails you&apos;ve hearted from the Recipe Vault.</p>
      </div>
      ...
      <div className={s.main}>
        ...
        {!loadingInitial && !error && recipes.length === 0 && (
          <p className={s.empty}>
            ...
            <Link to="/recipes">Recipe Vault</Link> and tap the heart on any cocktail.
          </p>
        )}
                ...
                actions={
                  <>
                    <RecipeFavoriteButton
                      ...
                      onChange={(next) => {
                        if (!next) removeFavorite(r.id)
                        else patchRecipe(r.id, { is_favorited: true })
                      }}
